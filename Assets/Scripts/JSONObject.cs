				...
				if(string.Compare(str, "true", true) == 0) {
					...
					try {
						...
						case '{':
							...
							keys = new List<string>();
							list = new List<JSONObject>();
							if (str[1] == '}') {return;} // Added by ryooo
							break;
						case '[':
		...
		case JSONObject.Type.OBJECT:
			if(list.Count > 0) {
				...
				for(int i = 0; i < list.Count; i++) {
					// Added keys.Count check by ryooo
					if (i < keys.Count) {
	...
	public Dictionary<string, string> ToDictionary() {
		if(type == Type.OBJECT) {
			...
			for(int i = 0; i < list.Count; i++) {
				...
				case Type.ARRAY:		result.Add((string)keys[i], val + "");	break;
				case Type.OBJECT:		result.Add((string)keys[i], val + "");	break;
				default:
